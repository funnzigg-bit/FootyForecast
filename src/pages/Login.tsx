...
const Login = () => {
  const testEmail = import.meta.env.VITE_TEST_USER_EMAIL?.trim() || "";
  const testPassword = import.meta.env.VITE_TEST_USER_PASSWORD?.trim() || "";
  const canUseTestSignIn = Boolean(testEmail && testPassword);
  ...
  const handleTestSignIn = async () => {
    if (!canUseTestSignIn) {
      toast({
        title: "Test sign-in unavailable",
        description: "Set VITE_TEST_USER_EMAIL and VITE_TEST_USER_PASSWORD to enable one-click access.",
        variant: "destructive",
      });
      return;
    }
    ...
    <div className="flex min-h-screen items-center justify-center bg-background bg-grid px-4">
      <div className="w-full max-w-md">
        ...
        <div className="rounded-xl border border-border bg-card p-8">
          ...
          <p className="mb-6 text-sm text-muted-foreground">Sign in to your FootyForecast account</p>

          {canUseTestSignIn && (
            <div className="mb-4 rounded-lg border border-primary/20 bg-primary/5 p-3">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-semibold text-foreground">Testing shortcut</p>
                  <p className="text-[11px] text-muted-foreground">One-click sign-in with the configured test account.</p>
                </div>
                <Button
                  type="button"
                  size="sm"
